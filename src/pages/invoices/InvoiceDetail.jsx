...
const InvoiceDetail = () => {
  ...
  const updateStatus = async (newStatus) => {
    ...
    try {
      ...
      // Mettre à jour en base de données avec délai maximum pour éviter les blocages
      await withTimeout(
        Invoice.updateStatus(invoice.id, newStatus),
        15000,
            ...
            ${invoice.parts.map((part, index) => `
              <tr style="border-bottom: 1px solid ${tpl.border};">
                ...
                <td style="padding: 10px; text-align: right;">
                  ${new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(part.quantity * part.unitPrice * (1 - (part.discount || 0) / 100))}
                  ${part.discount ? `<div style="font-size:10px;color:#555;">Remise appliquée : –${new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(part.quantity * part.unitPrice * (part.discount || 0) / 100)}</div>` : ''}
                </td>
              </tr>
            `).join('')}
          </tbody>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px; page-break-inside: avoid;">
          ...
          <tbody>
            ...
            <tr style="border-bottom: 1px solid ${tpl.border};">
                ...
                <td style="padding: 10px; text-align: right;">${new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(labor.hours * labor.rate)}</td>
              </tr>
            `).join('')}
            <tr style="border-bottom: 1px solid ${tpl.border}; background-color: ${tpl.sectionBg}; font-weight: bold;">
              ...
              <td style="padding: 10px; text-align: right;">${new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(invoice.laborDetails.reduce((sum, item) => sum + (item.hours * item.rate), 0))}</td>
            </tr>
          </tbody>
        </table>
        ` : `
  ...
  const calculateLaborTotal = () => {
    ...
    return invoice.laborDetails.reduce((total, labor) => total + (labor.hours * labor.rate), 0);
  };
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-card rounded-lg border">
            ...
            <div className="p-6" ref={invoiceRef}>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {invoice.parts && invoice.parts.map((part, index) => (
                      <tr key={index}>
                        <td className="py-3 px-3">{part.description}</td>
                        ...
                        <td className="py-3 px-3 text-right">
                          {formatCurrency(part.quantity * part.unitPrice * (1 - (part.discount || 0) / 100))}
                          {part.discount > 0 && (
                            <div className="text-xs text-muted-foreground">
                              Remise appliquée : –{formatCurrency(part.quantity * part.unitPrice * (part.discount || 0) / 100)}
                            </div>
                          )}
                        </td>
                      </tr>
                    ...
                    <tbody className="divide-y">
                      {invoice.laborDetails.map((labor, index) => (
                        <tr key={index}>
                          ...
                          <td className="py-3 px-3 text-right">{formatCurrency(labor.hours * labor.rate)}</td>
                        </tr>
                      ))}
              ...
              {/* Totaux */}
              <div className="mt-4 p-3 rounded-md bg-yellow-50 border border-yellow-200 text-sm font-sans text-gray-800">
                <p>
                  ✏️ Tu veux modifier un prix ou supprimer une ligne ? C’est simple : clique sur le bouton
                  <span className="font-semibold text-blue-600"> "Modifier"</span> en haut à droite !
                </p>
                <p className="mt-1">
                  🔍 Vérifie bien chaque ligne, surtout si le rapport est aussi long qu’un ticket de supermarché 🧾.
                  Il se peut qu’une petite erreur se cache… notre IA <span className="text-green-600 font-medium">s’améliore chaque jour</span> pour devenir infaillible !
                </p>
              </div>
